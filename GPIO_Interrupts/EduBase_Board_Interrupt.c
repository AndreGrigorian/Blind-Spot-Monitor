 ...
 *
 * @brief Source file for the EduBase_Button_Interrupt driver.
 *
...
void EduBase_Button_Interrupt_Init(void(*task)(uint8_t))
{
    ...
    // Configure the PD3 and PD2 pins to function as GPIO pins
    // by clearing Bits 3 to 2 in the AFSEL register
    GPIOD->AFSEL &= ~0x0C;
...
void GPIOD_Handler(void)
{
    if (GPIOD->MIS & 0x0C)
    {
        // Read the status of the PD3 and PD2 pins
        uint8_t edubase_button_status = GPIOD->DATA & 0x0C;

        // Execute the user-defined function and pass the status of the PD3 and PD2 pins
        (*EduBase_Button_Task)(edubase_button_status);

        // Acknowledge the interrupt from either the PD3 or the PD2 pin and clear it
        GPIOD->ICR |= 0x0C;
    }
}
